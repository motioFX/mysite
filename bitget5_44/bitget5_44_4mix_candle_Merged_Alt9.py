...
def plot_normalized_dataset(
    ...
    *,
    ...
    # 1日ごとの区切り線 (00:00) を追加
    try:
        tz_info = timestamps.dt.tz if hasattr(timestamps.dt, "tz") else None
        ...
        for day_start in day_starts:
            if timestamps.iloc[0] <= day_start <= timestamps.iloc[-1]:
                ax.axvline(day_start, color="gray", linestyle=":", linewidth=0.8, alpha=0.4, zorder=1)
    except Exception:
        pass
...
def score_and_plot_symbols(
    ...
    *,
    ...
    # 1. 新・地合い判定フェーズ (30d, 10d, 5d のノーマライズ始点比較の多数決)
    symbol_dfs = {}
    for symbol, csv_path, rows in results:
        if rows <= 0:
            continue
        try:
            ...
            symbol_dfs[symbol] = df_sym
        except Exception as e:
            log(f"Failed to read {csv_path}: {e}")

    # 地合い判定
    periods = [("30d", 30 * 24), ("10d", 10 * 24), ("5d", 5 * 24)]
    period_states = {}
    ...
    for label, hours in periods:
        ...
        for sym, df_sym in symbol_dfs.items():
            if df_sym is None or len(df_sym) < hours:
                continue
            close_s = df_sym["bybit_close"].astype(float).dropna()
            if len(close_s) < hours:
                continue
            c_start = float(close_s.iloc[-hours])
            c_now = float(close_s.iloc[-1])
            if c_start > 0:
                if (c_now / c_start) >= 1.0:
                    above_cnt += 1
                else:
                    below_cnt += 1
        st = "long" if above_cnt > below_cnt else "short"
        period_states[label] = st
        log(f"[market_state_v9] {label}: above={above_cnt}, below={below_cnt} -> {st.upper()}")

    long_votes = sum(1 for s in period_states.values() if s == "long")
    market_state = "long_only" if long_votes >= 2 else "short_only"
    log(f"[market_state_v9] Final Market State (Majority Vote): {market_state} (Long votes: {long_votes}/3)")

    # 2. 新・除外ルール判定 (30d, 10d, 5d の前半50%で最高値/最安値をマークした銘柄を除外)
    excluded_long = set()
    excluded_short = set()
    ...
    for label, hours in periods:
        ...
        for sym, df_sym in symbol_dfs.items():
            if df_sym is None or len(df_sym) < hours:
                continue
            df_w = df_sym.iloc[-hours:].reset_index(drop=True)
            close_s = df_w["bybit_close"].astype(float).dropna()
            if len(close_s) < hours:
                continue
            c_start = float(close_s.iloc[0])
            if c_start <= 0:
                continue
            norm_s = close_s / c_start
            max_idx = int(norm_s.idxmax())
            max_val = float(norm_s.max())
            high_records.append((max_val, max_idx, sym))

            min_idx = int(norm_s.idxmin())
            min_val = float(norm_s.min())
            low_records.append((min_val, min_idx, sym))

        # ノーマライズ最高値 Top 3
        high_records.sort(key=lambda x: x[0], reverse=True)
        for val, idx, sym in high_records[:3]:
            if idx < hours / 2:
                excluded_long.add(sym)
                log(f"[exclusion_v9] LONG除外 ({label}): {sym} (ノーマライズ最高値が前半 {idx}/{hours} で発生)")

        # ノーマライズ最安値 Top 3
        low_records.sort(key=lambda x: x[0], reverse=False)
        for val, idx, sym in low_records[:3]:
            if idx < hours / 2:
                excluded_short.add(sym)
                log(f"[exclusion_v9] SHORT除外 ({label}): {sym} (ノーマライズ最安値が前半 {idx}/{hours} で発生)")

    # 3. 新・重み付けスコアリング (30d, 10d, 5d のTop 3に倍率加算)
    scores = {sym: 0.0 for sym in symbol_dfs.keys()}
    is_long_mode = (market_state == "long_only")
    active_exclusions = excluded_long if is_long_mode else excluded_short

    weight_configs = [
        ("30d", 30 * 24, [1.2, 1.1, 1.0]),
        ("10d", 10 * 24, [1.5, 1.2, 1.0]),
        ("5d", 5 * 24, [2.0, 1.5, 1.0]),
    ]

    for label, hours, weights in weight_configs:
        norm_list = []
        for sym, df_sym in symbol_dfs.items():
            if sym in active_exclusions:
                continue
            if df_sym is None or len(df_sym) < hours:
                continue
            close_s = df_sym["bybit_close"].astype(float).dropna()
            ...
            c_now = float(close_s.iloc[-1])
            if c_start > 0:
                norm_perf = c_now / c_start
                norm_list.append((norm_perf, sym))

        if is_long_mode:
            norm_list.sort(key=lambda x: x[0], reverse=True)  # 上昇率高い順
        else:
            norm_list.sort(key=lambda x: x[0], reverse=False) # 下落率大きい(価格比低い)順

        for rank, (norm_perf, sym) in enumerate(norm_list[:3]):
            w = weights[rank]
            scores[sym] += w
            log(f"[scoring_v9] {label} Rank {rank+1} ({'LONG' if is_long_mode else 'SHORT'}): {sym} (norm={norm_perf:.4f}) -> +{w} pts")

    metrics_list = []
    for sym, df_sym in symbol_dfs.items():
        bg_turnover = float((df_sym["bybit_volume"] * df_sym["bybit_close"]).dropna().mean() * 24) if "bybit_volume" in df_sym.columns else 0.0
        bb_turnover = bg_turnover
        is_low_liq = 1 if bg_turnover < 5000000.0 else 0
        ban_s = 1 if (sym in excluded_short or is_low_liq) else 0

        metrics_list.append({
            "symbol": sym,
            "score": scores.get(sym, 0.0),
            "score_bitget": scores.get(sym, 0.0),
            "score_bybit": scores.get(sym, 0.0),
            "bg_daily_turnover": bg_turnover,
            "bb_daily_turnover": bb_turnover,
            "is_low_liq": is_low_liq,
            "ban_short": ban_s,
            "ban_long": 1 if sym in excluded_long else 0
        })
